...
class SerialLearningCalculator(Calculator):
    ...
    Determines when to switch between the physics and learnable calcualtor based
    on a skepticism signal from the learnable calculator.
    ...
    history_length: int
        ...
        function to use when establishing a link between skepticism parameter
    ...
    """History of pairs of the skepticism signal and observed error"""

    alpha: Optional[float] = None
    """Coefficient which relates distrust signal and observed error"""

    threshold: Optional[float] = None
    """Current threshold for the skepticism signal beyond which the target calculator will be used"""
    ...
    def calculate(
        ...
        self.used_surrogate = self.threshold is not None and skept_signal < self.threshold
        if self.used_surrogate:
            logger.debug(f'The skepticism signal is low enough ({skept_signal:.2e} < {self.threshold:.2e}). Using the surrogate result.')
